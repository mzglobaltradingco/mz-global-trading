import type { KnowledgePost } from "@/types/knowledge";

const post: KnowledgePost = {
  id: "5",
  slug: "pre-shipment-inspection-checklist-textile-buyers",
  title: "The Complete Pre-Shipment Inspection Checklist for Textile Buyers",
  excerpt:
    "A pre-shipment inspection is your last line of defence before goods leave the factory. Use this checklist to know exactly what should be checked — and what to do if it fails.",
  category: "Resources",
  tags: ["PSI", "Quality Control", "Inspection", "Checklist"],
  readTime: 6,
  date: "2024-01-09",
  author: "M. Muzammil",
  featured: false,
  image: "/knowledge/pre-shipment-inspection-checklist-textile-buyers.webp",
  content: `## Why Pre-Shipment Inspection Matters

A pre-shipment inspection (PSI) is your last opportunity to identify and resolve quality issues before goods leave the factory. Catching a defect at this stage costs significantly less than managing a return, a credit note or a customer complaint at destination.

## The Checklist

Use this as a starting point. Tailor it to your specific product and market requirements.

### Quantity Verification

- Total carton count matches packing list
- Units per carton match specification
- Total units shipped match the purchase order quantity

### Packaging and Labelling

- Carton markings correct (PO number, destination, gross/net weight)
- Care labels present, correct and securely attached
- Country of origin labelling compliant with destination market regulations
- Barcodes scannable and matching product codes

### Fabric and Construction

- GSM within agreed tolerance (±5%)
- Fabric composition matches specification
- Colour matches approved sample under standard light sources (D65 and TL84)
- Seam strength and stitch density within specification
- No pilling, snagging or surface defects on AQL sample

### Measurement

- Garment measurements within tolerance on all sizes
- Measurement taken relaxed, not stretched
- Minimum 3 units measured per size in the ratio

### Finishing

- No loose threads, missed stitches or skipped seams
- Buttons, zips and closures functional and secure
- Embellishments registered correctly and durably applied

> The AQL standard most widely used in textile export is AQL 2.5 for major defects. Agree your AQL level with your sourcing partner before production begins — not after.

## Working With Your Sourcing Partner

MZ Global Trading conducts basic production reviews as standard on all orders. Dedicated third-party inspection scope and associated costs are agreed on a per-order basis before production begins.`,
};

export default post;
